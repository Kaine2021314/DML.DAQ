...
namespace Voith.DAQ.Common
{
    public static class PlcConvert
    {
        ...
        {
            var str = GetCharsAt(Buffer, Pos, Size);

            str = str.Substring(2, Size - 2);//去掉起始符

            int endIndex = 0;
            //找到换行符作为截止位置
            if (str.IndexOf("\0", StringComparison.Ordinal) >= 0)
            {
                endIndex = str.IndexOf("\0", StringComparison.Ordinal);
            }
            if (str.IndexOf("\n", StringComparison.Ordinal) >= 0)
            {
                endIndex = endIndex < str.IndexOf("\n", StringComparison.Ordinal) && endIndex != 0
                    ? endIndex
                    : str.IndexOf("\n", StringComparison.Ordinal);
            }
            if (str.IndexOf("\r", StringComparison.Ordinal) >= 0)
            {
                endIndex = endIndex < str.IndexOf("\r", StringComparison.Ordinal) && endIndex != 0
                    ? endIndex
                    : str.IndexOf("\r", StringComparison.Ordinal);
            }

            //没有找到换行符并且取值长度小于等于读到的字符串长度，则不从换行符取值
            if (endIndex == 0 && Size - 2 <= str.Length)
            {
                endIndex = Size - 2;
            }
            else if (Size - 2 > str.Length)
            {
                endIndex = str.Length;
            }
